...
```{python}
#| label: alice-top-rated

alice_top_count = len(alice_ratings.filter(pl.col("rating") >= 4.5))
display(Markdown(f"**ALICE'S TOP-RATED MOVIES (rating >= 4.5, showing 10 of {alice_top_count}):**"))
display(alice_ratings.filter(pl.col("rating") >= 4.5).select(["title", "rating", "genres"]).head(10))
```
