...
from recsys_genai.sasrec import SASRec as _SASRec, train_sasrec
```
...
The SASRec class below inherits from `_SASRec` (imported from `recsys_genai.sasrec`), which provides the full transformer architecture. Your task is to implement the `predict_next` method for inference!

**Your task**: Fill in the `predict_next` method to generate top-K recommendations:
1. Set the model to evaluation mode and disable gradients
2. Get logits from the forward pass
3. Extract logits for the last position (most recent item)
4. Find the top-K items with highest scores

**Hints**:
- Use `self.eval()` and `torch.no_grad()`
- Use `self(seq)` to call the forward method
- Use `torch.topk()` to find top-K values
...
class SASRec(_SASRec):
    ...
    Inherits the full transformer architecture from the base implementation.
    You only need to implement the predict_next method for inference!
    """
    ...
    def predict_next(self, seq: torch.Tensor, k: int = 10) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # TODO: Get logits from forward pass
        # TODO: Extract logits for last position
        # TODO: Find top-K items and scores
        ...
```
...
## Solution: `predict_next` Implementation

Here's the complete implementation of the `predict_next` method:
...
3. **Forward pass**: `self(seq)` calls the inherited `forward()` method to get logits
